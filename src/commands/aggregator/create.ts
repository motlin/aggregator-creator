...
export default class AggregatorCreate extends Command {
	...
	private async fetchLatestLiftwizardVersion(): Promise<string> {
		const url = 'https://search.maven.org/solrsearch/select?q=g:io.liftwizard+AND+a:liftwizard-profile-parent&rows=1&wt=json';
			...
			https.get(url, options, (res) => {
				...
				res.on('end', () => {
					...
					try {
						...
						const latestVersion = jsonData.response.docs[0]?.latestVersion || jsonData.response.docs[0]?.v;
	...
	public async run(): Promise<{
		...
		if (!proceed) {
			...
			this.log(`│  │  │ - parent: ${chalk.yellow(`io.liftwizard:liftwizard-profile-parent:${liftwizardVersion}`)}`);
